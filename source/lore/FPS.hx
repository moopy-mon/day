	...
	**/
	...
	private override function __enterFrame(deltaTime:Float):Void
	{
		...
		if (currentCount != cacheCount /*&& visible*/)
		{
			...
			(ClientPrefs.showLore ? (!ClientPrefs.compactFPS ? "Moopy Monday " : "") + "v" + (MainMenuState.loreEngineVersion.endsWith(".0") ? MainMenuState.moopVer.replace(".0", "") : MainMenuState.moopVer) + MainMenuState.versionSuffix : "")
			#if debug + " (debug)" #end;
